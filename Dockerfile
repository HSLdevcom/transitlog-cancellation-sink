...
COPY --from=BUILD /usr/src/app/target/transitlog-event-source-jar-with-dependencies.jar /usr/app/transitlog-event-source.jar

ENTRYPOINT ["java", "-jar", "/usr/app/transitlog-event-source.jar"]
